...
function AvatarFallback({ name, size = 'md' }: { name: string; size?: 'sm' | 'md' }) {
  ...
  return (
    <div className={`${sizeClass} rounded-full bg-[#ee2b47] flex items-center justify-center border-2 border-white shadow-md`}>
    ...
    <div className="lg:hidden">
      ...
      `}>
        ...
        <div className="flex items-center gap-3 px-4 py-4 border-b border-[#f4f4f5]">
          ...
          <div>
            <p className="font-semibold text-[14px] text-[#18181b]">{user.name}</p>
            <p className="text-xs font-medium text-[#71717b] uppercase tracking-[0.3px]">{user.roleLabel}</p>
          </div>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className={`size-7 rounded-[10px] flex items-center justify-center shrink-0 ${isActive ? 'bg-[rgba(255,228,230,0.5)]' : ''}`}>
                  <Icon className={`size-4 ${isActive ? 'text-[#c70036]' : 'text-[#ee2b47]'}`} />
                </div>
                <span className={`text-sm ${isActive ? 'font-semibold text-[#c70036]' : 'font-medium text-[#52525c]'}`}>
                  {item.label}
                </span>
              </button>
            );
        ...
        <div className="px-3 pb-4 pt-1 border-t border-[#f4f4f5] space-y-1">
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-3 py-3 rounded-[12px] hover:bg-gray-100 transition-colors cursor-pointer"
          >
